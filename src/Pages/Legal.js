import React from 'react';
import '../styles/pages/legal.scss'
import Navbar from '../components/Header/Navbar';

function Legal() {
  return (
    <section className="page-legal">
      <Navbar />
      <div className="legal-content">
          <div className="legale-content-parag">
            <h2>Mentions Légales</h2>
            <br />
            <p>Nom de l'entreprise :</p>
            <p>Adresse du siège social :</p>
            <p>Numéro de téléphone :</p>
            <p>Adresse e-mail :</p>
            <br />
            <p>Directeur de publication :</p>
            <p>Nom :</p>
            <p>Adresse e-mail :</p>
            <br />
            <p>Hébergeur du site :</p>
            <p>Nom :</p>
            <p>Adresse :</p>
            <p>Numéro de téléphone :</p>
            <p>Adresse e-mail :</p>
            <br />
            <p>Nom de domaine :</p>
            <p>Propriétaire :</p>
            <p>Adresse :</p>
            <br />
            <p>Numéro d'identification de l'entreprise :</p>
            <p>Registre du Commerce et des Sociétés (RCS) :</p>
            <br />
            <p>Cookies :</p>
            <p>Nous tenons à vous informer que notre site web ne collecte pas de cookies. Nous ne stockons pas d'informations personnelles sur nos visiteurs, à l'exception des informations que vous nous fournissez volontairement via nos formulaires de contact ou d'inscription. Nous respectons votre vie privée et nous nous engageons à protéger vos données personnelles conformément aux lois en vigueur en matière de protection des données. Si vous avez des questions ou des préoccupations concernant la manière dont nous traitons vos données personnelles, veuillez nous contacter en utilisant le formulaire de contact sur notre site web.</p>
            <br />
            <p>Confidentialité</p>
            <p>Chacune des parties s'engage à ne pas divulguer, ni communiquer, ni laisser divulguer ou laisser communiquer, ni utiliser directement ou indirectement, à moins qu'il n'y ait été autorisé préalablement et par écrit par l'autre partie, les renseignements, données, informations, applications, méthodes et savoir-faire à caractère confidentiel ainsi que tout document de quelque nature que ce soit dont il a connaissance à l'occasion de l'exécution de ses prestations. Chacune des parties s'engage à faire respecter cette obligation de confidentialité par ses salariés et collaborateurs.</p>
            <br />
            <p>Propriété intellectuelle :</p>
            <p>Tous les éléments du Site, y compris les textes, les images, les graphismes, les logos, les vidéos, les sons, les marques, les logiciels et les bases de données, sont protégés par le droit d'auteur et/ou le droit des marques. Toute reproduction, représentation, utilisation ou adaptation, sous quelque forme que ce soit, de tout ou partie des éléments du Site sans l'autorisation écrite préalable de [nom de l'entreprise] est strictement interdite et constitue une contrefaçon sanctionnée par le Code de la propriété intellectuelle.</p>
            <br />
            <p>Limitation de responsabilité :</p>
            <p>Nous ne pouvons être tenus responsables des dommages directs, indirects ou consécutifs, y compris la perte de profits, de revenus, de données ou de clients, résultant de l'utilisation ou de l'incapacité d'utilisation du Site et des Services, ou de tout contenu ou information obtenu à partir du Site ou des Services.</p>
            <br />
            <p>Loi applicable et juridiction compétente :</p>
            <p>Les présentes mentions légales sont régies et interprétées conformément aux lois françaises. Tout litige relatif à l'utilisation du Site ou des Services sera soumis à la compétence exclusive des tribunaux français.</p>
            <br />
            <p>Nous nous réservons le droit de modifier les présentes mentions légales à tout moment et sans préavis. Veuillez consulter régulièrement cette page pour prendre connaissance des modifications éventuelles.</p>
          </div>

      </div>
    </section>
  );
}

export default Legal;